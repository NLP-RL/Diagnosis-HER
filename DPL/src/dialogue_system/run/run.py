...
parser.add_argument("--saved_model", dest="saved_model", type=str, default="./../../../../Diagnosis_HER_Results/model/DQN/checkpoint/0107164252_agenthrljoint2_T22_ss100_lr0.0005_RFS66_RFF0_RFNCY0.0_RFIRS0_RFRA-44_RFRMT-66_mls0_gamma0.95_gammaW0.9_epsilon0.1_awd0_crs0_hwg0_wc0_var0_sdai0_wfrs44.0_dtft0_ird0_ubc0.985_lbc1e-10_data_RID0/model_d12agenthrljoint2_s0.833_r38.016_t8.402_mr0.201_mr2-0.33_e-4009.pkl")
parser.add_argument("--save_dialogue", dest="save_dialogue", type=boolean_string, default=False, help="Save the dialogue? [True, False]")
parser.add_argument("--dialogue_file", dest="dialogue_file", type=str, default="", help="the file that used to save dialogue content.")
parser.add_argument("--path_for_disease_record_storage", dest="path_for_disease_record_storage", type=str, default="./../../../../Diagnosis_HER_Results/data/SS-DHER-9/", help="the path location where the disease record and other files are stored")
...
# choice of dataset : by default, SD dataset will be used.
parser.add_argument("--dataset_type", dest="dataset_type", type=str, default="MD", help="Which dataset is being utilized? SD or MD")

# goal set, slot set, action set.
max_turn = 22
file0='./../../classifier/data/md_dataset'
...
# HER methods configuration
parser.add_argument("--is_trimming_used", dest="is_trimming_used", type=bool, default=True, help="Do we apply trimming DDA method while training the model? True or False")
parser.add_argument("--is_stitching_used", dest="is_stitching_used", type=bool, default=False, help="Do we apply stitching DDA method while training the model? True or False")

# reward design
parser.add_argument("--reward_for_not_come_yet", dest="reward_for_not_come_yet", type=float,default=0.0)
parser.add_argument("--reward_for_success", dest="reward_for_success", type=float,default=4* max_turn)
parser.add_argument("--reward_for_fail", dest="reward_for_fail", type=float,default=-10)
...
parser.add_argument("--reward_model_1", dest="reward_model_1", type=boolean_string, default=False, help="top 3 disease rewards are 66, 55, 44; shaping = 44")
parser.add_argument("--reward_model_2", dest="reward_model_2", type=boolean_string, default=False, help="top 3 disease rewards are 66, 55, 44; shaping = 11")
parser.add_argument("--reward_model_3", dest="reward_model_3", type=boolean_string, default=False, help="top 3 disease rewards are 66, 55, 33; shaping = 44")
parser.add_argument("--reward_model_4", dest="reward_model_4", type=boolean_string, default=False, help="top 3 disease rewards are 66, 55, 33; shaping = 11")
parser.add_argument("--reward_model_5", dest="reward_model_5", type=boolean_string, default=False, help="top 4 disease rewards are 66, 60, 50; shaping = 44") 
...
def run(parameter):
    ...
    if train_mode is True: # Train
        ...
        for index in range(simulate_epoch_number):
            if parameter.get("dataset_type") == "MD":
                steward.evaluate_model(dataset='validate', index=index)
            else:
...
if __name__ == "__main__":
    ...
    if parameter.get("reward_model_2") or parameter.get("reward_model_4"):
        parameter["weight_for_reward_shaping"] = 11.0

    #checkpoint_path = './../../../../Diagnosis_HER_Results/model/DQN/checkpoint/'

    if parameter.get("dataset_type") == "MD":
        parameter["disease_number"] = 12
        dataset_location = './../../classifier/data/md_dataset'
        parameter["file_all"] = dataset_location
        parameter["action_set"] = dataset_location + '/action_set.p'
        parameter["slot_set"] = dataset_location + '/slot_set.p'
        parameter["goal_set"] = dataset_location + '/goal_set.p'
        parameter["disease_symptom"] = dataset_location + "/disease_symptom.p"
