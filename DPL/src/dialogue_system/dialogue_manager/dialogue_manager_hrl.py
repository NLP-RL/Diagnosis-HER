...
class DialogueManager_HRL(object):
    ...
    def __init__(self, user, agent, parameter):
        ...
        if self.parameter.get("dataset_type") == "SD":
            self.acc_by_group = {x:[0,0,0] for x in ['12', '13', '14', '19', '1', '4', '5', '6', '7']}
        else:
            self.acc_by_group = {x:[0,0,0] for x in range(1, 13)}

        # It holds the name of the top 3 diseases predicted
        self.top_3_diseases = []
        ...
        if self.parameter.get("train_mode")==False:
            ...
            if self.parameter.get("dataset_type") == "SD":
                ...
                self.top_k_disease_record = {'1': [], '2': [], '3': [], '4': [], '5': []}
                self.test_by_group = {str(x):[0,0,0] for x in range(1, 13)}
                self.disease_record = []
                self.lower_reward_by_group = {str(x): [] for x in range(1, 13)}
                # self.master_index_by_group = {str(x):[] for x in range(1, 13)}
                self.master_index_by_group = []
                self.symptom_by_group = {str(x): [0,0] for x in range(1, 13)}
    ...
    def next(self, greedy_strategy, save_record, index):
        ...
        if self.parameter.get("disease_as_action")==False:
            ...
            if action_type == "disease" or condition:# or lower_action in self.lower_action_history:
                ...
                if self.parameter.get("is_trimming_used"):
                    
                    # Identifying the numeric IDs of the top 3 predicted diseases
                    model_outputs = Ys.detach().cpu().numpy()
                    sorted_model_outputs_indices_top_3 = np.argsort(model_outputs,axis=1)[:, -3:]
                    top_3_disease_ids = sorted_model_outputs_indices_top_3.tolist()[0]

                    # Identifying the names of the top 3 predicted diseases (3rd, 2nd, 1st) order
                    self.top_3_diseases = [self.id2disease[i] for i in top_3_disease_ids]
        ...
        # Applying trimming process if its flag is True and end of episode is reached
        if len(self.top_3_diseases) == 3 and self.parameter.get("is_trimming_used") is True and episode_over is True:

            # Identifying the true disease of the user goal
            true_disease_of_goal = self.state_tracker.user.goal["disease_tag"]

            # Performing trimming by partial reward model
            if true_disease_of_goal == self.top_3_diseases[0]: # If true disease is the 3rd one in top-3 predicted diseases
                reward = 33
                print('partital reward top3')
            elif true_disease_of_goal == self.top_3_diseases[1]: # If true disease is the 2nd one in top-3 predicted diseases
                reward = 44
                print('partital reward top3')
            else: # Other cases, do nothing
                reward = -10
                print('penalty')
    ...
    def initialize(self, dataset, goal_index=None):
        ...
        # Clearing and setting the default value to the top 3 disease predictions
        self.top_3_diseases = []
    ...
    def train_deep_learning_classifier(self, epochs):
        ...
        if self.parameter.get("train_mode"):
            ...
            test_batch = random.sample(self.disease_replay, min(1000,len(self.disease_replay)))
            test_acc = self.model.test(test_batch=test_batch)
            print('disease_replay:{}, test_acc:{:.4f}'.format(len(self.disease_replay), test_acc))
            self.dc_accuracy.append(test_acc)
